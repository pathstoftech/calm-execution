...
| A11Y-002 | P1       | Home / Detail images   | Runtime images were not visually displayed. Home announced `imageKey`; Detail showed image description text instead of the actual image.                                       | Fixed: Home and Detail now render real WebP images through `DrawableTipImageResolver` and `CalmTipImage`. Home no longer exposes raw image keys. Detail uses catalog image descriptions as image content descriptions when images are non-decorative. Verified manually on Pixel 9a and Medium Tablet; JVM, compact, and expanded test buckets passed. | Fixed before D5-04 Done                 | Closed |
